...
def get_exchange_rate_from_juhe():
    ...
    try:
        ...
        if data.get('error_code') == 0:
            ...
            if usd_cny_data:
                ...
                logging.info(f"成功从聚合数据获取汇率数据：{rate_data['name']} - {rate_data['price']}")
                return rate_data
            else:
...
def get_exchange_rate_from_mxnzp():
    ...
    try:
        ...
        if data.get('code') == 1:
            ...
            logging.info(f"成功从美心智能平台获取汇率数据：{rate_data['name']} - {rate_data['price']}")
            return rate_data
        else:
